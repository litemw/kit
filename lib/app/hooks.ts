...
export function createContainerHooks(logger: Logger): ContainerHooks {
  return {
    ...
    onFactoryStart(component) {
      logger.debug('Factory started for component {component}', { component: component.name });
    },
    ...
    onCacheHit(component) {
      logger.debug('Cache hit for component {component}', { component: component.name });
    },
    onMultiResolve(token, count) {
      logger.debug('Multi token {token} resolved', { token: token.name, count });
    },
    onError(error, context) {
      const err = Err(error);
      logger.error(err, 'Container error, token: {token}', { token: context.token?.name });
    },
  };
}
